...
void ScreenEffect::Draw()
{
	...
	if (m_fade_cnt > 0)
	{
		...
		if (m_color == FadeColor::Black){
			GraphicsManager::GetInstance()->DrawBox(0, 0, 1280.0f, 720.0f, (D3DCOLOR_COLORVALUE(0.0f, 0.0f, 0.0f, alpha)));
		}
		// 白アルファブレンド
		if (m_color == FadeColor::White){
			GraphicsManager::GetInstance()->DrawBox(0, 0, 1280.0f, 720.0f, (D3DCOLOR_COLORVALUE(1.0f, 1.0f, 1.0f, alpha)));
		}
	}
