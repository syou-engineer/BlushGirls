...
Runa::Runa(int index)
{
	...
	// テクスチャのコピー
	m_hDown = m_texData->GetRunaAnim(6);
	m_hHit = m_texData->GetRunaAnim(7);
	m_hJump = m_texData->GetRunaAnim(8);
	m_hSquat = m_texData->GetRunaAnim(15);
	m_hWalk = m_texData->GetRunaAnim(19);
	m_hWin[0] = m_texData->GetRunaAnim(20);
	m_hWin[1] = m_texData->GetRunaAnim(21);

	// ３枚アニメーションを一括でコピー
	for (int i = 0; i < 3; i++){
		m_hJumpAttack[i] = m_texData->GetRunaAnim(i + 9);
		m_hNeutral[i] = m_texData->GetRunaAnim(i + 12);
		m_hSquatAttack[i] = m_texData->GetRunaAnim(i + 16);
	}
	for (int i = 0; i < 6; i++){
		m_hAttack[i] = m_texData->GetRunaAnim(i);
	}
...
*/
void Runa::Initialize(int index)
{
	switch (index)
	{
	case PLAYER1:
...
void Runa::Advance()
{
	SetSpeed(GetChrPrm->RunaAdvanceSpeed);
}
...
void Runa::GuardNow()
{
	...
	case 0:
		...
		SetSpeed(GetChrPrm->RunaGuardSpeed);
...
* @brief 上段攻撃が当たったか
*/
bool Runa::isUpperAttackHit(const IPlayer& enemy)
{
	ARGB red = { 255, 255, 0, 0 };
...
* @brief 下段攻撃が当たったか
*/
bool Runa::isLowerAttackHit(const IPlayer& enemy)
{
	// 下段攻撃じゃなかったら何もしない
...
* @brief 被弾
*/
void Runa::Damage(const IPlayer& attacker, AttackPoint hitType)
{
	switch (hitType)
	{
...
* @brief 位置を設定
*/
void Runa::SetPos(const Vector2& pos)
{
	m_obj.pos = pos;
}
...
* @brief 状態を設定
*/
void Runa::SetState(const State state)
{
	m_state = state;
}
...
* @brief 向きを設定
*/
void Runa::SetDirection(const Dir& dir)
{
	m_obj.direction = dir;
}
...
* @brief プレイヤー番号を取得
*/
const unsigned int Runa::GetID() const
{
	return m_id;
}
...
* @brief 攻撃力を取得
*/
const float Runa::GetPower() const
{
	return m_power;
}
...
* @brief 位置を取得
*/
const Vector2& Runa::GetPos() const
{
	return m_obj.pos;
}
...
* @brief 状態を取得
*/
const State Runa::GetState() const
{
	return m_state;
}
...
* @brief 攻撃範囲を取得
*/
const Rect Runa::GetAttackRect() const
{
	return m_attackRect;
}
...
* @brief ヒット範囲を取得
*/
const Rect Runa::GetHitRect() const
{
	return m_hitRect;
}
...
* @brief 向きを取得
*/
const Dir Runa::GetDirection() const
{
...
void Runa::Dash()
{
	SetSpeed(GetChrPrm->RunaDashSpeed);
}
...
void Runa::BackStep()
{
	if (m_subFrame == 0){
		...
		SetSpeed(GetChrPrm->RunaBackStepSpeed);
	}
...
void Runa::SquatAttack()
{
	...
	case 0: // 初期化
		...
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackSquatPower;
		// ボイスの再生
		SoundPlayer::GetInstance()->Play(RUNA_SQUATATTACK_VOICE);
...
void Runa::DashAttack()
{
	...
	case 0:
		...
		// 攻撃力の設定
		m_power = GetChrPrm->RunaDashSpeed;
		// ボイスを再生
		SoundPlayer::GetInstance()->Play(RUNA_DASHATTACK_VOICE);
		// 制御不可
		m_isControll = false;
		// アニメーションセット
		m_animation = 1;
...
void Runa::JumpAttack()
{
	...
	case 0:
		...
		m_attackPoint = AttackPoint::Upper;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackJumpPower;
		// アニメーションの設定
		m_animation = 0;
		// 効果音とボイスの再生
		SoundPlayer::GetInstance()->Play(SWORDSWING_SE);
		SoundPlayer::GetInstance()->Play(RUNA_JUMPATTACK_VOICE);
		// エフェクトを重ねて描画する
...
void Runa::WeakAttack()
{
	...
	case 0:
	{
			  ...
			  m_attackPoint = AttackPoint::Upper;
			  // 攻撃力の設定
			  m_power = GetChrPrm->RunaAttackWeakPower;

			  // 音声の再生
			  SoundPlayer::GetInstance()->Play(RUNA_ATTACKWEAK_VOICE);
			  SoundPlayer::GetInstance()->Play(SWORDSWING_SE);
...
* @brief 弱攻撃派生技
*/
void Runa::WeakAttackDerivation()
{
	switch (m_subWork)
	{
	case 0:
		...
		m_attackPoint = AttackPoint::Upper;
		// 攻撃力を設定
		m_power = GetChrPrm->RunaAttackWeakDerivationPower;
		// 当たり判定を生成
		m_isAttackJudge = true;
		m_attackRect = { 0, 0, 200, 200 };
		// 速度の設定
		...
		m_animation = 1;
		// ボイスを再生する
		SoundPlayer::GetInstance()->Play(RUNA_ATTACKWEAK_DER_VOICE);
		// 操作不可
		m_isControll = false;
...
void Runa::MediumAttack()
{
	...
	case 0: // 初期化
		...
		m_attackPoint = AttackPoint::Upper;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackMediumPower;
		SoundPlayer::GetInstance()->Play(RUNA_JUMPATTACK_VOICE);
		// 制御不能
		m_isControll = false;
		// アニメーションの設定
...
void Runa::StrongAttack()
{
	...
	case 0: // 初期化
		...
		m_isControll = false;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackStrongPower;
		// 当たり判定の生成
		m_isAttackJudge = true;
		m_attackRect = { 0, 0, 200, 200 };
		// アニメーションの設定
		...
		// 乱数で再生するボイスを変更する
		if (r == 0) SoundPlayer::GetInstance()->Play(RUNA_ATTACKSTRONG_VOICE);
		if (r == 1) SoundPlayer::GetInstance()->Play(RUNA_ATTACKSTRONG_VOICE2);
		// 速度の設定
		SetSpeed(GetChrPrm->RunaAttackStrongSpeed);
...
* @brief 強攻撃派生技
*/
void Runa::StrongAttackDerivation()
{
	switch (m_subWork)
	{
	case 0:
...
void Runa::LongAttack()
{
	...
	case 0:
		...
		m_attackPoint = AttackPoint::Upper;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackLongPower;
		SoundPlayer::GetInstance()->Play(RUNA_ATTACKWEAK_VOICE);
		m_isControll = false;
		//	m_spFlag = false;
		m_animation = 5;
...
void Runa::SpecialAttack()
{
	...
	case 0: // 初期化
		...
		m_isControll = false;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackSpecialPower;
		// カットインを設定する
		CutInManager::GetInstance()->Create(RUNA, m_id, m_obj.pos);

		// ボイスの再生
		SoundPlayer::GetInstance()->Play(RUNA_ATTACKSPECIAL_VOICE2);
	...
	case 2: // ボイスの再生
		SoundPlayer::GetInstance()->Play(RUNA_ATTACKSPECIAL_VOICE3);
		m_subWork++;
		adjustFrame = 10;
	...
	case 3:
		...
		if (adjustFrame <= 0)
		{
			...
			m_slashingCount++;
			// 16回以下なら処理を続ける
			if (m_slashingCount > GetChrPrm->RunaSlashingCountMax){
				m_subWork++;
			}
		}
		break;
	...
	case 4: // 必殺技追撃状態に入る前にパラメータをリセットしておく
		...
		SoundPlayer::GetInstance()->Play(RUNA_ATTACKSPECIAL_PUR_VOICE);
		break;
	}
	// フレームを加算
	m_subFrame++;
...
*/
void Runa::SpecialPursuitAttack()
{
	switch (m_subWork)
	{
	case 0:
		...
		m_isAttackJudge = true;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackSpecialPursuitPower;
		// アニメーションの設定
		m_animation = 1;
		// 速度の設定
		SetSpeed(GetChrPrm->RunaAttackSpecialPursuitSpeed);
		m_subWork++;
		break;
	...
	case 1:
		...
		if (m_subFrame > 40){
			...
			// 攻撃が成功
			if (m_isAttackSuccess){
				SoundPlayer::GetInstance()->Play(RUNA_ATTACKSPECIAL_SUCCESS);
			}
			else {
				ActionReset();
				SetState(State::Neutral);
...
void Runa::RaidAttack()
{
	...
	case 0:
		...
		m_isControll = false;
		// ボイスの作成
		SoundPlayer::GetInstance()->Play(RUNA_ATTACKMEDIUM_VOICE);
		...
		m_animation = 0;
		// 攻撃力の設定
		m_power = GetChrPrm->RunaAttackRaidPower;
		m_subWork++;
		break;
...
void Runa::Passive()
{
	...
	case 1: // 速度とボイスを設定する
		...
		SoundPlayer::GetInstance()->Play(RUNA_PASSIVE_VOICE);
		m_subWork++;
		break;
...
void Runa::BeforeBattle()
{
	...
	case 0:
		...
		// 操作不能にする
		m_isControll = false;
		SoundPlayer::GetInstance()->Play(RUNA_BEFOREBATTLE_VOICE);
		m_subWork++;
		break;
...
void Runa::Win()
{
	...
	case 0:
		...
		m_isControll = false;
		m_animation = 0;
		SoundPlayer::GetInstance()->Play(RUNA_WIN_VOICE);
		m_subWork++;
		break;
...
*/
void Runa::SetSpeed(const float spx, const float spy)
{
	if (m_obj.direction == Dir::Right){
		m_spd.x = spx;
	}
...
void Runa::AddAngle(float speed)
{
...
void Runa::SpecialAttackCreateEffect()
{
	float temp, temp2;
	...
	if (m_slashingCount > GetChrPrm->RunaSlashingCountMax - 1){
		subWork = 0;
		workFrame = 0;
		adjustWorkFrame = 10;
	}
