...
BOOL DirectSound::Load(LPSTR fileName)
{
	...
	map<string, LPDIRECTSOUNDBUFFER>::iterator it = m_soundBuffer.find(fileName);
	...
	HMMIO hMmio = NULL; // WindowsマルチメディアAPIのハンドル(WindowsマルチメディアAPIはWAVファイル関係の操作用のAPIでありDirectXではない）
